...
| POST | `/v1/trust/anchor-changes/sync` | 跨系统信任锚点变更流同步接收与防重放检查点（验真后落盘、不记审计）：请求体恰含 `changes`（JSON 对象）、`after`（非布尔非负整数），缺失/非法 JSON/非对象/键集或类型错误均 400 且仅 `{"error":非空中文}`，显式空租户头 400、缺省 `default`、按租户隔离；外层合法后完整复用 `/v1/trust/anchor-changes/verify` 的变更结构、锚点用途、签名格式与验签顺序，四类失败均 **200** `{"valid":false,"reason"}` 且不写入；验真成功后以 `(租户, signer_did)` 为检查点键：首个非空页须 `after=0`，续页须等于已存 `next_after`；同 `after` 且 `changes` 递归键升序紧凑 UTF-8 JSON 字节相同为幂等重放，旧页/跳页/同位异内容均 **409** 仅 `{"error":"同步游标冲突"}`；首个非空页 **201**、后续新页 **200**，重放或空页 **200** 且不推进；成功响应键序恰为 `valid,signer_did,next_after,accepted`（`valid=true`，新页 `accepted` 为 events 长度，重放或空页为 0）；每个新页将原始 events、摘要与检查点原子落盘，落盘失败全部回滚并 **500** 仅 `{"error":"存储失败"}`；并发同一检查点至多一项推进，重启后结论不变 |
