...
| GET | `/v1/trust/anchor-changes/synced-state?signer_did=&at=&limit=&after=` | 只读锚点变更同步视图：查询参数仅此四项且均只能出现一次，`signer_did` 必填非空，`at` 缺省取该签名方同步检查点、须为 ASCII 非负整数，`limit` 缺省 50、须为 1–200 的 ASCII 整数，`after` 缺省 0、须为 ASCII 非负整数；空值、重复、未知参数、非 ASCII 数字、符号、越界或 `after>at` 均 400 且仅 `{"error":非空中文}`；来源未同步或跨租户 404 同形，`at` 超过检查点 409 同形；取已同步事件中 `cursor≤at` 者按 `(did,key_version)` 以末项为准合并，再按 `last_cursor>after` 升序取至多 `limit` 项；200 按键序恰返 `signer_did`、`at`、`anchors`、`next_after`，`anchors` 项键序恰为 `did`、`key_version`、`public_key`、`status`（active/revoked）、`uses`（规范序）、`last_action`、`last_cursor`；空页 `next_after` 等于 `after`，否则取末项 `last_cursor`；同一 `at` 的视图不受后续同步影响，重启逐字节一致；纯只读、不推进检查点、不写状态或审计 |
