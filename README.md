...
| POST | `/v1/trust/credentials/verify-with-status` | 外部凭证验真+状态判定合并查询（只读）：请求体与验真规则同 `/v1/trust/credentials/verify`，**任何失败均 HTTP 200** 且保持相同失败优先级；验签通过后按本租户 `(issuer_did, credential_id)` 查同步记录——未同步 `外部凭证状态未同步`、`active` 仅返回 `{"valid":true}`、`revoked` 返回 `外部凭证已吊销：<保存 reason，缺省 未知原因>`、`unknown` 返回 `外部凭证状态未知`；不创建凭证、不改状态、不写同步记录或审计 |
