...
| POST | `/v1/trust/credentials/import` | 验签通过后导入/持久化外部凭证；请求体恰为 `{body,signature}`，`body` 依 `/v1/trust/credentials/verify` 规则必含 `credential_id`/`issuer_did`/`subject_did`/`claims`/`issued_at`，`signature` 为非空字符串；请求/字段错 400 仅 `{"error":...}`；锚点缺失/非 active、签名格式错、验签失败 **HTTP 200** 返回 `{"valid":false,"reason":...}` 且不写入；首次按 `tenant+issuer_did+credential_id` 保存 201，响应键序 `imported,issuer_did,credential_id,body,signature`（`imported:true`）；同内容重放 200 回原响应、不重复审计，不同内容 409 仅 `{"error":...}` |
| GET | `/v1/trust/credentials/imported/{credential_id}?issuer_did=...` | 读取已导入的外部凭证（只读）；`issuer_did` 须唯一非空，缺失/重复/空值 400；未导入、跨租户或不匹配 404；成功 200 键序 `issuer_did,credential_id,body,signature`；持久化且重启可读 |
