...
| GET | `/v1/trust/ac-proof?cursor=&snapshot=&signer_did=` | 只读生成锚点变更流事件的 Merkle 包含证明：查询参数仅允许唯一 `cursor`、唯一 `snapshot` 与唯一非空 `signer_did`；`cursor`、`snapshot` 为 ASCII 十进制正整数且 `cursor≤snapshot≤本租户变更流最大游标`，缺失/空值/重复/空白/符号/小数/布尔词/Unicode 数字/越界/未知参数均 400 且仅 `{"error":非空中文}`；`signer_did` 与租户错误沿用变更流入口（未知含他租户 404、已停用 409，400 判定优先）；`cursor` 对应事件不存在 404；以 cursor 升序的 snapshot 前缀建树：叶=SHA-256(0x00\|\|事件规范 JSON 的 UTF-8)，父=SHA-256(0x01\|\|左\|\|右)，奇数末项复制；200 键序 `event,snapshot,root,path,signer_did,signer_key_version,signature`，`event` 沿用变更流事件协议，`root` 为 64 位小写 hex，`path` 自叶向根、项键序 `side,hash`（`side` 限 `left`/`right`，`hash` 同格式），`signature` 由签名 DID 当前私钥按既有 ES256 裸 R\|\|S 无填充 base64url 规则签前六键；纯只读、不记审计 |
| POST | `/v1/trust/ac-proof` | 跨系统锚点变更包含证明只读验真（不依赖本地事件、不写状态/游标/审计）：请求体须恰含 `proof` 对象，否则 400 仅 `{"error":非空中文}`；`proof` 须恰含 GET 响应七键；依次校验结构、本租户同 `signer_did`/`signer_key_version` 且含 `generic` 用途的 active 锚点、签名格式、密码学签名，再按 `path` 自叶向根重算 `root`；失败均 HTTP 200 按键序 `{valid,reason}`，原因恰为“证明非法”/“锚点不可用”/“签名格式错误”/“签名校验失败”/“包含证明校验失败”；成功仅 `{"valid":true}`；租户头缺省 `default`、显式空 400 并隔离，重启稳定 |
