...
| POST | `/v1/credentials/{credential_id}/present` | 生成选择性披露演示，请求体恰为 `{"disclose":[路径...]}` 加可选 `challenge`、`expires_in`；成功 201 返回演示对象（含 `challenge`、`expires_at`）；字段问题 400、未知凭证 404 |
| POST | `/v1/presentations/{presentation_id}/verify` | 校验演示，新演示请求体恰为 `{"presentation":对象,"challenge":串}`；**任何失败一律 HTTP 200**，成功 `{"valid":true}`（一次性消费），失败附非空中文 `reason` |
...
curl -X POST localhost:8080/v1/credentials/vc_<id>/present \
  -d '{"disclose":["/role","/addr/city"]}'   # [] 为零披露；可选 challenge/expires_in
curl -X POST localhost:8080/v1/presentations/vp_<id>/verify \
  -d '{"presentation":{...},"challenge":"<上一步返回的 challenge>"}'
```
...
- `POST /v1/credentials/{credential_id}/present` 请求体必须**恰为**
  ...
  `disclose`、不是数组、含多余字段一律 400；未知凭证 404。
  `[]` 表示**零披露**。
- `challenge` 缺省生成 32 位小写 hex；提供时须为非空字符串且按
  Unicode 码点最多 256。`expires_in` 缺省 300 秒；提供时须为非布尔
  整数且在 1–86400。非法字段 400。
- 响应在原有字段上增加 `challenge` 与 `expires_at`；`expires_at` 为
  持久化 UTC 秒加 `expires_in`（Z 结尾秒精度），二者均写入演示并
  签入 `proof`。
...
- `proof` 为 **ES256**、无填充 base64url 的裸 `R||S` 签名，覆盖**除 `proof`
  外按 key 升序规范化 JSON**（含 `challenge` 与 `expires_at`），使用凭证
  `issuer_key_version` 对应的**历史版本私钥**；因此签发者轮换密钥后，
  旧演示仍可用历史公钥验真。
- 演示记录随状态文件持久化在 `presentations` 中，**重启后仍可验签**。
- `POST /v1/presentations/{presentation_id}/verify`：新演示请求体必须恰为
  `{"presentation":对象,"challenge":串}` 且 `challenge` 为非空字符串；
  旧演示（存储记录缺少 `challenge`）接受恰含 `{"presentation":对象}` 的
  请求，不检查挑战、过期与消费。采用与凭证验签相同的公开错误协议：
  **任何失败都返回 HTTP 200** 与 `{"valid":false,"reason":"<非空中文原因>"}`。
  校验顺序：请求、资源 ID、绑定（含请求 challenge、演示 challenge 与
  proof 覆盖的存储 challenge 三者一致）、已消费、过期（当前时间大于等于
  `expires_at`）、投影、proof 格式与签名、吊销。已消费优先于过期与吊销；
  签名成功但已吊销返回 `凭证已吊销：<原因>` 且**不消费**。仅未过期、
  未吊销且验签成功才返回 200 `{"valid":true}`（无其他字段）并**原子标记
  已消费**：并发只能一次成功，过期或失败不消费，消费记录跨重启保留，
  重复验证返回 `演示已消费`。

```bash
curl -X POST localhost:8080/v1/credentials/vc_<id>/present \
  -d '{"disclose":["/role","/addr/city"],"challenge":"verifier-nonce","expires_in":600}'
curl -X POST localhost:8080/v1/presentations/vp_<id>/verify \
...
越界/非法 JSON/challenge 与 expires_in 边界）、未知凭证 404、演示 verify
的全部分类失败与中文 reason、challenge 三方一致、一次性消费（并发恰一次
成功、失败与过期不消费、重复验证判已消费、消费记录跨重启保留）、过期判定、
吊销不消费、轮换后历史版本验签、旧演示（无 challenge）兼容验证。
...
vcbackend/
  ...
  models.py    DIDRecord / CredentialRecord / CredentialStatusRecord /
               PresentationRecord 数据模型
  store.py     文件持久化存储、DID 去重、密钥轮换、凭证签发/验签与
               选择性披露演示（RFC6901 路径校验、claims 投影）
