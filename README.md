...
| POST | `/v1/trust/credentials/verify-receipt` | 跨系统凭证验真签名回执（只读）：请求体恰含 `body`、`signature`、`verifier_did`、`nonce`，`verifier_did` 须非空字符串、`nonce` 须为 1–256 码点非空字符串；非法 JSON、非对象、键集不符或两者非法均 **400** 且仅 `{error}`。请求级合法后 `body`、`signature` 完全沿用既有外部凭证验真全规则，任一失败均先 **200** 恰返 `{valid:false,reason}` 并沿用原原因，且不查询验证者 DID；仅成功才查本租户验证者 DID：未知或跨租户 **404**、停用 **409**，均仅 `{error}`。成功 **200** 键序恰为 `valid`、`receipt`、`receipt_signature`；`receipt` 按序恰含 `credential_id`、`issuer_did`、`issuer_key_version`、`credential_digest`、`verifier_did`、`verifier_key_version`、`nonce`，两版本为正整数（`issuer_key_version` 缺省按 1），`credential_digest` 为 `{body,signature}` 递归键升序紧凑 UTF-8 JSON 的 SHA-256 小写 64 位 hex；`receipt_signature` 由验证者**当前私钥**按既有 ES256 裸 `R||S` 无填充 base64url 规则签 `receipt`。纯只读、不落盘、不记审计 |
