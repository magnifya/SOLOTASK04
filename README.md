...
| POST | `/v1/trust/credentials/imported/{credential_id}/verify?issuer_did=...` | 重启后对已落盘凭证重新验签（纯只读）：请求体必须恰为 `{}`（空体、非法 JSON、非对象或含字段均 400）；`issuer_did` 须唯一非空，缺失/重复/空值 400；未导入、错配或跨租户 404。取存储 `body`、`signature` 原文，`issuer_key_version` 缺省按 1，用同 DID/版本的 active 锚点对 `body` 递归键升序紧凑 UTF-8 JSON 做 ES256 验签（签名须为无填充 base64url 的 64 字节裸 R‖S）。锚点缺失/吊销 HTTP 200 `{"valid":false,"reason":"锚点不可用"}`，格式错“签名格式错误”，验签错“签名校验失败”，`expires_at` 已达到“凭证已过期”；成功仅 `{"valid":true}`。不写记录、状态、历史或审计，不泄露私钥，吊销/重启/他租户结论稳定 |
