...
| POST | `/v1/trust/credentials/receipt/consume-batch` | 批量验真并消费跨系统凭证验真签名回执（防重放）：请求体须恰为 `{"items":[项...]}`，数组非空且不超过 100 项；空体、非法 JSON、非对象、键集错误、`items` 非数组/空/超限均 **HTTP 200** 按序恰返 `{"results":[],"reason":"请求…"}`；合法批次逐项处理、失败不短路，每项恰含 `receipt`、`receipt_signature`、`body`、`signature`、`nonce`，类型与 nonce 规则（非空串、nonce 1–256 码点）沿用单项，项结构非法返 `{"valid":false,"reason":"请求项非法"}`，其余复用单项七阶段验真顺序、固定 `reason` 及优先级；验真通过后按租户以 `(verifier_did,nonce)` 判重，批内首项成功、后项或历史重放返 `{"valid":false,"reason":"回执已消费"}`；成功项键序恰为 `valid`、`receipt_id`、`consumed_at`，取值沿用单项，`results` 与输入等长、同序；本批全部新消费标记与审计在同一把锁内经同一次原子写落盘（全为重放则不写盘），落盘失败整批回滚并 **500** 仅返 `{"error":"存储失败"}`；与单项 consume 并发时每键仅一次成功，重启仍判重；租户头缺省 `default`、显式空 400、按租户隔离；单条回执接口及 CLI 不变 |
