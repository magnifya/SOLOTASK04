...
| POST | `/v1/trust/anchor-changes/verify` | 跨系统信任锚点变更流只读验真：请求体须恰为 `{"changes":对象,"after":非布尔非负整数}`，否则 **400** 且仅 `{"error":非空中文}`；`changes` 须恰含 GET 变更流响应的 `events`、`next_after`、`signer_did`、`signer_key_version`、`signature` 五键，字段与事件协议沿用该入口（事件恰含 `cursor,action,did,key_version,public_key,status,uses`，`events` 至多 200 项、`cursor` 严格递增且均大于 `after`，非空时 `next_after` 等于末项 `cursor`、空时等于 `after`）；外层合法后任何失败均 **HTTP 200**，依次校验结构、本租户同 `signer_did`/版本且含 `generic` 用途的 active 锚点、签名格式、签名（ES256 裸 `R||S` 无填充 base64url，覆盖 `changes` 除 `signature` 外四键的规范化 JSON），按键序恰返 `valid:false`、`reason`，原因依次恰为“变更流非法”“锚点不可用”“签名格式错误”“签名校验失败”，成功仅 `{"valid":true}`；不依赖本地事件，不写状态、游标或审计；租户头缺省 `default`、显式空值 400 并隔离，结论跨重启稳定 |
