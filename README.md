...
| POST | `/v1/trust/credentials/receipt/consume` | 验真回执防重放消费：请求与字段约束（恰含 `receipt`、`receipt_signature`、`body`、`signature`、`nonce`，`nonce` 1–256 码点非空串）沿用 `/v1/trust/credentials/receipt/verify`；外层非法（非法 JSON、非对象、键集或类型不符、显式空租户头）一律 **400** 且仅 `{error}`；外层合法后先按七阶段顺序验真（回执非法→nonce错误→绑定错误→摘要错误→锚点不可用→签名格式错误→签名校验失败），失败沿用原 **HTTP 200** 响应与固定 `reason`，不写状态、不记审计；验真成功后按租户以 `(verifier_did,nonce)` 为唯一键判重：首次 **200** 按序恰返 `valid`、`receipt_id`、`consumed_at`，`valid:true`，`receipt_id` 为 `receipt` 规范化 JSON 字节（递归键升序紧凑 UTF-8）的 SHA-256 小写 64 位 hex，`consumed_at` 为 UTC 秒精度 Z 字符串；首次消费与审计（`action=trust.credential.receipt.consumed`、`resource_type=credential_receipt`、`resource_id=receipt_id`）在同一次原子写落盘；同键重放（`receipt` 可不同）**200** 恰返 `{"valid":false,"reason":"回执已消费"}`，不替换、不审计，并发仅一次成功；落盘失败回滚消费记录与审计、**500** 仅返 `{"error":"存储失败"}` 且可重试；重启后仍判重；租户头缺省 `default`、按租户隔离；不重验凭证签名 |
