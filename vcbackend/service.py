...
def build_handler(store: VCStore) -> type:
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def _post_trust_credentials_verify_synced_batch_with_status(
            ...
            # （只读）。请求体须恰含 signer_did（非空字符串）、at
            # （非布尔非负整数）、credentials（1–100 项数组）；空体、
            # 非法 JSON、非对象、键集或类型错误、空数组或超限均 400
            # 且仅 {"error":"请求非法"}；来源未同步或跨租户 404 仅
            # {"error":"同步来源不存在"}，at 超检查点 409 仅
            # {"error":"同步游标冲突"}，均先于项校验。合法时 200 仅返
            # {"results":[...]}，逐项不短路、等长同序：项须恰含 body
            # （对象）与非空 signature，否则该项
            # {"valid":false,"reason":"请求项非法"}；合法项沿用
            # verify-synced-batch 的凭证字段、版本兼容、cursor<=at
            # 末锚点、签名、期限、校验顺序及原因。验真通过后按
            # (issuer_did, credential_id) 查批初原子读取的本租户状态
            # 快照：未同步、revoked、unknown 分别返“外部凭证状态未
            # 同步”“外部凭证已吊销：<reason>”（空原因用“未知原因”）
            # “外部凭证状态未知”；active 成功。成功项仅
            # {"valid":true}，失败项键序 valid、reason。纯只读：不写
            # 状态或审计；租户头缺省 default、显式空 400 并隔离。
            try:
                length = int(self.headers.get("Content-Length") or 0)
                raw = self.rfile.read(length) if length > 0 else b""
            except Exception:  # noqa: BLE001 请求非法绝不暴露内部细节
