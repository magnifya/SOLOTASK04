...
def _receipt_consumption_ndjson_bytes(events: Any) -> bytes:
    """按导出端点的确定性规则将回执消费事件编码为 NDJSON 字节。

    每行键序固定为 cursor、receipt_id、verifier_did、nonce、
...
def build_handler(store: VCStore) -> type:
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def _get_trust_credential_receipt_consumptions_export(
            ...
            # GET /v1/trust/credentials/receipt/consumptions/export：
            # 确定性 NDJSON 导出本租户验真回执消费历史，支持快照续传。
            ...
            # verifier_did、nonce、consumed_at（cursor 为正整数，其余为
            # 字符串），UTF-8 紧凑 JSON、非 ASCII 不转义、RFC8259 最短
            # 转义、LF 结行（末行亦有 LF）、无 BOM，空结果零字节。同一
            # snapshot 续页天然排除快照后新事件，重启字节一致。纯只读：
            # 不改游标、状态或审计。
            params = parse_qs(query, keep_blank_values=True)
            ...
            events, snapshot_cursor, next_after = (
                store.export_receipt_consumptions(
                    tenant,
                    after,
                    limit,
                    snapshot=snapshot,
