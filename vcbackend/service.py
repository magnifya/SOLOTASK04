...
路由：
  ...
  POST /v1/trust/proofs/verify-with-status 外部谓词证明验真并合并同步状态（只读）
  POST /v1/trust/proofs/verify-batch-with-status 批量证明验真并合并同步状态（只读）
...
def build_handler(store: VCStore) -> type:
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def _post_trust_proofs_verify_with_status(
            ...
            # 返回 200 + {"valid": false, "reason": "<非空中文原因>"}。
            # 请求体须恰含 proof（对象）、challenge 与 source_tenant_id
            # （非空字符串），解析规则与 /v1/trust/proofs/verify 完全
            # 一致；验真通过后由 store 只读查询本租户同步状态。纯只读，
            # 不消费、不写状态/历史、不记审计。
            try:
                length = int(self.headers.get("Content-Length") or 0)
                raw = self.rfile.read(length) if length > 0 else b""
        ...
        def _post_trust_proofs_verify_batch_with_status(
            ...
            # 批量外部谓词证明验真并合并本租户同步状态：与 verify-batch
            # 相同的公开错误协议，任何失败都返回 HTTP 200。请求体须恰为
