...
def build_handler(store: VCStore) -> type:
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def _get_did_history(
            ...
            # GET /v1/dids/{did}/history?limit=&after=：只读 DID 注册/
            # 停用历史。响应恰含 did、events、next_after；事件恰含
            # action、status、reason、updated_at、audit_seq、
            # audit_timestamp、cursor，按 cursor 升序。查询参数仅允许
            # limit、after：limit 缺省 50、须为 1..200 的非空 ASCII
            # 十进制整数；after 缺省 0、须为非空非负 ASCII 十进制整数；
            # 重复、空白、布尔词、小数、符号、Unicode 数字及未知参数一律
            # 400。未知或他租户 DID 404；空页 next_after 保持 after。
            # 纯只读：不写任何状态、不记审计。
            if not did:
                raise ValidationError("路径缺少 did")
