...
def build_handler(store: VCStore) -> type:
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def _post_trust_dids_verify_document_synced(self, tenant: str) -> None:
            # POST /v1/trust/dids/verify-document-synced：以同步锚点快照
            # 验真外部 DID 文档（只读）。请求体须恰含 signer_did（非空
            # 字符串）、at（非布尔非负整数）、document（JSON 对象）；
            # 空体、非法 JSON、非对象、键集或类型错误均 400 且仅
            # {"error":"请求非法"}。signer_did 未同步或跨租户 404 仅
            # {"error":"同步来源不存在"}，at 超过同步检查点 409 仅
            # {"error":"同步游标冲突"}，均先于文档校验。合法时按文档、
            # 锚点、签名格式、验签、停用通告顺序校验，任何失败均 200
            # 按键序恰返 {"valid":false,"reason":...}：文档结构沿用既
            # 有 DID 文档验真协议（四字段、验证方法升序无重、P-256
            # PEM、禁私钥、当前版本最高、proof 覆盖），失败为
            # “DID文档非法”；锚点取该来源 cursor<=at 的 (did, 当前
            # 版本) 末事件，缺失、非 active、uses 无 did 或公钥不逐字
            # 相同均为“同步锚点不可用”；格式、验签失败依次为“签名
            # 格式错误”“签名校验失败”；验签成功后命中本租户外部
            # DID 停用通告为“外部DID已停用：<reason>”；成功仅
            # {"valid":true}。纯只读：不改同步页、检查点、锚点、状态
            # 或审计，重启一致；租户头缺省 default、显式空 400 并隔离。
            try:
                length = int(self.headers.get("Content-Length") or 0)
                raw = self.rfile.read(length) if length > 0 else b""
            ...
            if (
                ...
                return
            try:
                valid, reason = store.verify_trust_did_document_synced(
                    tenant, signer_did, at, document
                )
        ...
        def _post_trust_dids_verify_document_synced_batch(
            ...
            # {"results":[...]}，逐项不短路、等长同序：每项即一份 DID
            # 文档，非对象项失败原因为“DID文档非法”，其余逐项沿用
            # verify-document-synced 的文档结构、cursor<=at 末锚点、
            # ES256 proof 覆盖、校验顺序与停用判定，失败原因恰为
            # “DID文档非法”/“同步锚点不可用”/“签名格式错误”/
            # “签名校验失败”/“外部DID已停用：<reason>”；成功项仅
            # {"valid":true}，失败项键序 valid、reason。批初原子读取
            # 同步视图与本租户停用通告，并发写入不造成批内混合结论。
            # 纯只读：不写数据、不推进检查点、不审计，重启一致；租户头
            # 缺省 default、显式空 400 并隔离。
            try:
                length = int(self.headers.get("Content-Length") or 0)
                raw = self.rfile.read(length) if length > 0 else b""
            except Exception:  # noqa: BLE001 请求非法绝不暴露内部细节
        ...
        def _post_trust_presentations_consume(self, tenant: str) -> None:
            # POST /v1/trust/presentations/consume：验真并一次性消费
            # 跨系统外部演示（防重放）。
            # 1) 请求体解析、字段约束与验真规则完全沿用
            #    /v1/trust/presentations/verify：未绑定恰含
            #    presentation/challenge；持有者绑定另恰含非空
            #    source_tenant_id，演示恰为绑定十二字段，双锚点双签名、
            #    期限、校验顺序与 reason 全部一致；任何验真失败均
            #    HTTP 200 按键序仅返 {"valid":false,"reason":...}，不
            #    写消费标记、不记审计；显式空 X-Tenant-ID 在进入前由
            #    路由统一判 400；
            # 2) 验真成功后按租户以 (issuer_did, presentation_id) 为
            #    唯一键消费：首次 200 按序恰返 valid、consumption_id、
            #    consumed_at（valid:true；consumption_id 为请求规范化
            #    JSON 字节的 SHA-256 小写 64 位 hex；consumed_at 为
            #    UTC 秒精度 Z）；同键重放（演示内容可不同）或并发均
            #    200 仅返 {"valid":false,"reason":"外部演示已消费"}，
            #    每键仅一次成功；
            # 3) 首次消费标记与审计（trust.presentation.consumed /
            #    trust_presentation / consumption_id）同一次原子写落盘；
            #    重放与验真失败不记；落盘失败回滚二者，500 仅返
            #    {"error":"存储失败"}，可重试；按租户隔离，跨重启仍
            #    判重。
            try:
                length = int(self.headers.get("Content-Length") or 0)
                raw = self.rfile.read(length) if length > 0 else b""
