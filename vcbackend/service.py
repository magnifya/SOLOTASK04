...
路由：
  ...
  GET  /v1/credentials/{credential_id}/status/history  查询本地状态历史（只读）
...
def build_handler(store: VCStore) -> type:
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_GET(self) -> None:  # noqa: N802
            try:
                ...
                if path == "/v1/audit":
                    ...
                    credential_id = unquote(
                        path[len("/v1/credentials/") : -len("/status/history")]
                    )
        ...
        def _get_credential_status_history(
            ...
            # GET /v1/credentials/{credential_id}/status/history?limit=&after=：
            # 只读本地凭证状态历史。limit 缺省 50，须为 1..200 的 ASCII
            # 十进制整数；after 缺省 0，须为非负 ASCII 十进制整数；
            # 重复/空白/布尔词/小数/符号/Unicode 数字一律 400。未知或
            # 他租户凭证 404；已签发但未登记状态的凭证返回空页，空页
            ...
            events, next_after = store.list_credential_status_events(
