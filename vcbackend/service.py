...
路由：
  ...
  POST /v1/dids/{did}/deactivate          DID 生命周期停用（幂等，记 did.deactivated）
  GET  /v1/dids/{did}/status              查询 DID 生命周期状态（active/deactivated，只读）
...
def build_handler(store: VCStore) -> type:
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def _post_deactivate_did(self, tenant: str, did: str) -> None:
            ...
            # 请求体仅允许空体、{} 或恰含可选 reason；非空时必须恰含
            # reason，其值由 store 在确认非重复停用后校验（字符串裁剪后
            # 非空，否则 400），重复停用时任何 reason（含非法值）均忽略
            # 并返回首次结果。未知或他租户 DID 404。首次 200 返回
            # did/status:"deactivated"/reason/updated_at，幂等同样 200。
            if not did:
        ...
        def _get_did_status(self, tenant: str, did: str) -> None:
            ...
            # 活动 DID 返回 status:"active"、reason/updated_at 均为 null；
            # 停用后返回首次停用的 reason 与 UTC 秒精度 Z 时间。未知或
            # 他租户 DID 404。纯只读：不改变状态与审计。
            if not did:
                raise ValidationError("路径缺少 did")
