...
def build_handler(store: VCStore) -> type:
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_POST(self) -> None:  # noqa: N802 (BaseHTTPRequestHandler API)
            try:
                ...
                if path == "/v1/dids":
                    ...
                    path
                    == "/v1/trust/credentials/verify-batch-with-status"
                ):
        ...
        def _post_trust_credentials_verify_batch_with_status(
            ...
            # 批量外部凭证验真并合并本租户同步状态：与批量纯验真相同的
            # 公开错误协议，任何失败都返回 HTTP 200。请求体须恰为
            # {"credentials": [项...]}，数组非空且不超过 100 项；请求体
            # 非法（含非法 JSON、空数组、超过上限）时返回
            # {"results": [], "reason": "请求..."}。请求级合法时逐项
            # 复用单项 verify-with-status 规则（验真 + 只读合并同步状
            # 态），按输入顺序收集结果，失败不短路。只读，不写凭证、状
            # 态、历史或审计。
            try:
                length = int(self.headers.get("Content-Length") or 0)
                raw = self.rfile.read(length) if length > 0 else b""
