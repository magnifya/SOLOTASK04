...
def build_handler(store: VCStore) -> type:
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def _post_trust_proofs_verify_synced_batch(
            ...
            # POST /v1/trust/proofs/verify-synced-batch：批量以同步
            # 锚点快照验真外部谓词证明（只读）。请求体须恰含
            # signer_did（非空字符串）、at（非布尔非负整数）、
            # proofs（1–100 项数组）；空体、非法 JSON、非对象、键集
            # 或类型错误、空数组或超限均 400 且仅
            # {"error":"请求非法"}；来源未同步或跨租户 404 仅
            # {"error":"同步来源不存在"}，at 超检查点 409 仅
            # {"error":"同步游标冲突"}，均先于项校验。合法时 200 仅
            # 返 {"results":[...]}，逐项不短路、等长同序：项须恰含
            # proof（对象）、challenge（非空字符串）与
            # source_tenant_id（非空字符串），否则该项
            # {"valid":false,"reason":"请求项非法"}；合法项沿用单条
            # verify-synced 的证明九字段、谓词、results、挑战、签名
            # 覆盖及“证明→挑战→锚点→格式→验签→期限”顺序，按该来源
            # cursor<=at 的 (DID,版本) 末项验真，锚点缺失/非
            # active/uses 无 proof 为“同步锚点不可用”，格式、验签、
            # 过期依次为“签名格式错误”“签名校验失败”“证明已过期”；
            # 成功项仅 {"valid":true}，失败项键序 valid、reason。
            # 纯只读：不改同步页、检查点、锚点、证明、状态或审计，
            # 重启一致；租户头缺省 default、显式空 400 并隔离。
            try:
                length = int(self.headers.get("Content-Length") or 0)
                raw = self.rfile.read(length) if length > 0 else b""
            except Exception:  # noqa: BLE001 请求非法绝不暴露内部细节
