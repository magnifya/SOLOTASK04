...
def build_handler(store: VCStore) -> type:
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def _get_key_history(
            ...
            # GET /v1/dids/{did}/keys/history?limit=&after=：只读
            # 密钥生命周期历史。分页规则与 keys/revocations 完全一致
            # （limit 缺省 50、限 1..200；after 缺省 0、须非负；
            # 重复/空白/布尔词/小数/符号/Unicode 数字一律 400）。未知或
            # 他租户 DID 404；已有 DID 无历史返回空页，空页
            # next_after 保持 after。响应事件仅含公开字段，绝不暴露私钥。
            # 纯只读：不写任何状态、不记审计。
            if not did:
                raise ValidationError("路径缺少 did")
            params = parse_qs(query, keep_blank_values=True)
            ...
            events, next_after = store.list_key_history(
                tenant, did, after, limit
            )
            self._send_json(
                200,
