...
路由：
  ...
  POST /v1/trust/proofs/verify            跨系统谓词证明验真（无需登记 DID/凭证/证明）
...
def build_handler(store: VCStore) -> type:
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def _post_trust_proofs_verify(self, tenant: str) -> None:
            # 跨系统谓词证明验真：与其他验签端点相同的公开错误协议，
            # 任何失败都返回 200 + {"valid": false, "reason": "<非空
            # 中文原因>"}。请求体须恰含 proof（对象）、challenge 与
            # source_tenant_id（均为非空字符串）；非法 JSON/非对象/
            # 缺失/多余字段均为请求类原因。只读，不消费、不写状态、
            # 不记审计。
            try:
                length = int(self.headers.get("Content-Length") or 0)
                raw = self.rfile.read(length) if length > 0 else b""
