...
GET  /v1/dids/{did}/keys/history        查询 DID 密钥生命周期历史（只读）
  ...
  POST /v1/trust/presentations/verify-synced  以同步锚点验真未绑定演示（只读）
...
def build_handler(store: VCStore) -> type:
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def _post_trust_presentations_verify_synced(
            ...
            # 类型错误均 400 且仅 {"error": 非空中文}。signer_did 未同
            # 步或跨租户 404、at 超过同步检查点 409，同形仅 {"error"}。
            # 外层合法后任何失败均 HTTP 200 按键序恰返
            # {"valid":false,"reason":...}：演示九字段、challenge、
            # expires_at、proof 覆盖及“请求→演示→挑战→锚点→签名格式
            # →验签→期限”顺序沿用 /v1/trust/presentations/verify 的未
            # 绑定形态（holder_* 字段按演示非法）；锚点取该来源
            # cursor<=at 的已同步事件、以 (issuer_did, 版本) 最后事件
            # 为准，缺失、非 active 或 uses 无 vp 均为“同步锚点不可
            # 用”；proof 格式错、验签失败、到期依次为“签名格式错
            # 误”“签名校验失败”“演示已过期”，较早错误优先；成功仅
