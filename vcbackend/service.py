...
def build_handler(store: VCStore) -> type:
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_POST(self) -> None:  # noqa: N802 (BaseHTTPRequestHandler API)
            try:
                ...
                if path == "/v1/dids":
                    ...
                    path
                    == "/v1/trust/credentials/receipt/consumptions/manifest/verify"
                ):
                    self._post_trust_receipt_consumptions_manifest_verify(
                        tenant
                    )
        ...
        def do_GET(self) -> None:  # noqa: N802
            try:
                ...
                if path == "/v1/audit":
                    ...
                    self._get_trust_receipt_consumptions_manifest(
                        tenant, parsed.query
                    )
        ...
        def _get_trust_receipt_consumptions_manifest(
            self, tenant: str, query: str
        ) -> None:
            # GET /v1/trust/credentials/receipt/consumptions/manifest：
            # 对一次确定性导出（与 export 同参数，snapshot 必填）生成
            # 签名摘要清单，协议复用停用通告清单。查询参数仅允许
            # limit、after、snapshot、signer_did，且均只能出现一次：
            # limit 缺省 1000（1..10000）、after 缺省 0（非负）、
            # snapshot 必填（非负且不得超过当时最大游标，after 不得
            # 大于生效 snapshot）、signer_did 必填非空。空值、重复、
            # 未知参数、格式或范围非法一律 400 且仅 {"error": 非空中文}；
            # 签名 DID 未知（含他租户）404、已停用 409（400 越界判定
            # 优先）。200 键序恰为 snapshot、filters、count、alg、
            # digest、signer_did、key_version、signature；filters 键
            # 序 after、limit，值为生效整数；count 为本页非负整数行
            # 数；alg 恒为 SHA-256；digest 为本页 NDJSON 字节（与
            # export 同形状）的 64 位小写 hex SHA-256；signature 由
            # 签名 DID 当前私钥对前七键规范化 JSON 做 ES256 裸 R||S
            # 无填充 base64url 签名。纯只读：不改游标、状态或审计。
            params = parse_qs(query, keep_blank_values=True)
        ...
        @staticmethod
        def _receipt_manifest_is_well_formed(manifest: Any) -> bool:
            # 回执消费清单结构校验（阶段一“清单非法”）：恰含八键且
            # 类型/取值合法；filters 恰含 after、limit 且均为生效整数。
            if not isinstance(manifest, dict):
                return False
            expected = {
                "snapshot",
        ...
        def _post_trust_receipt_consumptions_manifest_verify(
            self, tenant: str
        ) -> None:
            # POST /v1/trust/credentials/receipt/consumptions/manifest/verify：
            # 校验一次回执消费历史导出清单与其 NDJSON 内容。外层请求错
            # 误（非法 JSON/非对象/缺漏或多余字段/manifest 非对象/
            # ndjson 非字符串）一律 400 且仅 {"error": ...}。外层合法
            # 后任何失败均 HTTP 200，按既有清单验真顺序返回
            # {"valid":false,"reason":...}：清单非法 -> 锚点不可用
            # （本租户同 did/版本且含 vc 用途的 active 锚点）-> 签名
            # 格式错误 -> 签名校验失败 -> 导出内容不匹配（UTF-8
            # SHA-256 摘要及 LF 行数）。成功仅 {"valid":true}。
            # 纯只读、租户隔离、不记审计。
            data = self._read_json()
            if set(data) != {"manifest", "ndjson"}:
                missing = [f for f in ("manifest", "ndjson") if f not in data]
        ...
        def _verify_receipt_consumption_manifest_item(
            ...
            # 回执消费清单验真：按序返回失败原因（清单非法 -> 锚点不
            # 可用 -> 签名格式错误 -> 签名校验失败 -> 导出内容不匹
            # 配），成功返回 None。纯只读。
            # 阶段一：清单结构
            if not self._receipt_manifest_is_well_formed(manifest):
                return "清单非法"

            signer_did = manifest["signer_did"]
            key_version = manifest["key_version"]

            # 阶段二：本租户同 did/版本 active 信任锚点（须含 vc
            # 用途，否则按锚点不可用处理）
            public_pem = store.get_active_trust_anchor_public_key(
