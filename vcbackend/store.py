...
class VCStore:
    ...
    def _load(self) -> None:
        ...
        for bucket in self._tenants.values():
            ...
            bucket.setdefault("consumed_presentations", {})
    ...
    def _synced_anchor_events_latest_locked(
        ...
        """``_synced_anchor_events_latest`` 的锁内核心（调用方须持锁）。

        检查点语义与异常同上；返回的末事件视图引用锁内状态，调用方
        须在释放锁前完成使用或复制。
        """
    ...
    def consume_trust_presentation(
        ...
        """按 (issuer_did, presentation_id) 消费跨系统演示（防重放）。

        调用方须先完成演示验真（沿用 /v1/trust/presentations/verify 的
        全部检查）。本方法在锁内原子判定并标记：
        - 同键已消费：返回 (False, None)，不写状态、不记审计；
        - 首次消费：记录 consumed_at（UTC 秒精度 Z）并追加一条
          trust.presentation.consumed 审计（resource_type 为
          trust_presentation、resource_id 为 consumption_id），二者
          同一次原子写落盘，返回 (True, consumed_at)；并发仅一次
          成功，跨重启保留；
        ...
        with self._lock:
            bucket = self._bucket_locked(tenant_id)
            by_issuer = (
                bucket["consumed_presentations"].get(issuer_did)
                if bucket is not None
                else None
            )
            ...
            try:
                consumed_at = _utc_now()
                bucket["consumed_presentations"].setdefault(
                    issuer_did, {}
                )[presentation_id] = {
    ...
    def _verify_synced_did_document_against(
        ...
        """在已解析的同步锚点快照与停用通告快照上验真一份外部 DID
        文档（只读）。

        文档结构、cursor<=at 末锚点、ES256 proof 覆盖、校验顺序与
        停用判定的规则及原因分类与
        :meth:`verify_trust_did_document_synced` 完全一致，供单条与
        批量同步验真共用同一快照。
        """
        # 1. 文档结构：沿用既有 DID 文档验真协议
        parsed = VCStore._parse_trust_did_document(document)
        if parsed is None:
        ...
        # 5. 验签成功后查批初停用通告快照
        notice_reason = notice_reasons.get(did)
        if notice_reason is not None:
            return (
                False,
                f"{EXTERNAL_DID_DEACTIVATED_REASON_PREFIX}{notice_reason}",
            )
        return True, ""

    def _synced_did_verification_snapshot(
        self,
        tenant_id: str,
        signer_did: str,
        at: int,
    ) -> Tuple[Dict[Tuple[str, int], Dict[str, Any]], Dict[str, str]]:
        """同一次锁持有期间联合复制同步验真所需的两份快照（只读）。

        检查点语义与 :meth:`_synced_anchor_events_latest` 一致：来源
        无检查点（含跨租户）抛 NotFoundError，at 超过检查点抛
        ConflictError，均先于任何文档校验。合法时在同一锁持有期间
        复制该来源 cursor<=at 的同步锚点末事件视图（深拷贝，释放锁
        后的并发写入不影响副本）与本租户外部 DID 停用通告
        （did -> 通告 reason），随后立即释放锁；调用方在锁外逐项
        验真，验签期间不持锁。两份快照取自同一原子瞬间，并发原子
        更新（如同时撤销锚点并登记同 DID 停用通告）不会造成单条或
        批内混合结论。
        """
        with self._lock:
            ...
            notice_reasons = {
                did: row["reason"] for did, row in rows.items()
            }
            # 深拷贝末事件视图：锁外验真只读副本，与后续并发写入隔离
    ...
    def verify_trust_did_document_synced(
        ...
        #    同一次锁持有期间联合复制锚点末事件视图与停用通告快照
        latest, notice_reasons = self._synced_did_verification_snapshot(
            tenant_id, signer_did, at
        )
        # 2. 文档结构、锚点、签名与停用判定（锁外共用同一联合快照）
    ...
    def verify_trust_did_documents_synced_batch(
        ...
        联合复制 cursor<=at 的同步锚点末事件视图与本租户停用通告
        快照，释放锁后逐项验真（验签期间不持锁），整批共用同一原子
        瞬间的联合快照，并发原子写入（如同时撤销锚点并登记同 DID
        停用通告）不会造成批内混合结论；
