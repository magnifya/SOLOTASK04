...
class VCStore:
    ...
    def _verify_synced_proof_against(
        ...
        假定 proof 为 JSON 对象、challenge 与 source_tenant_id 为非空
        字符串；证明九字段、谓词、results、挑战、锚点、签名格式、验签
        与期限规则及原因分类与 :meth:`verify_trust_proof_synced` 完全
        一致。
        """
    ...
    def verify_trust_proofs_synced_batch(
        ...
        - 检查点语义与 :meth:`verify_trust_proof_synced` 一致：来源
          未同步（含跨租户）抛 NotFoundError，at 超检查点抛
          ConflictError；合法时按 cursor<=at 解析一次快照，整批共用；
        - 逐项不短路、等长同序：项须恰含 proof（JSON 对象）、
          challenge（非空字符串）与 source_tenant_id（非空字符串），
          否则该项 ``{"valid": false, "reason": "请求项非法"}``；
          合法项复用单条 verify-synced 的证明九字段、谓词、results、
          挑战、锚点、签名覆盖、格式、验签与期限规则及原因分类；
        - 成功项仅 ``{"valid": true}``，失败项键序为 valid、reason。
