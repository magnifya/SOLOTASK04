...
class VCStore:
    ...
    def verify_trust_presentation_synced(
        ...
        - 演示字段、challenge、expires_at 与 proof 覆盖范围沿用
          ...
          九字段，出现任何 holder_* 字段即失败；请求 challenge 须等于
          演示 challenge；proof 覆盖去掉 proof 与全部 holder_* 字段后
          的八个字段；
        - 锚点取该来源 cursor<=at 的已同步事件，以 (issuer_did,
          issuer_key_version) 最后事件为准；缺失、非 active 或 uses
          无 vp 均返回 (False, "同步锚点不可用")；
        - 签名须为 ES256、64 字节裸 R||S 无填充 base64url；格式错、
        ...
        # 签名格式（严格：64 字节裸 R||S 无填充 base64url）与密码学
        # 验签：覆盖范围沿用本地绑定演示——去掉 proof 及全部 holder_*
        # 字段后的八个字段（未绑定演示本就不含 holder_*）。
        proof = presentation["proof"]
        message = {
            k: v
            for k, v in presentation.items()
