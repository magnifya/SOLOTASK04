...
class VCStore:
    ...
    def verify_trust_proof_synced(
        ...
        - 该签名方在本租户无同步检查点（含跨租户）抛 NotFoundError；
          at 超过检查点抛 ConflictError；
        - 证明字段、谓词、结果、挑战与期限规则沿用
          :meth:`verify_trust_proof`：proof 须恰为九字段
          （proof_id、credential_id、issuer_did、issuer_key_version、
          predicates、results、challenge、expires_at、proof），请求
          challenge 须等于证明对象 challenge；
        - 锚点取该来源 cursor<=at 的已同步事件，以 (issuer_did, 版本)
          最后事件为准；缺失、非 active 或 uses 无 proof 均返回
          (False, "同步锚点不可用")；
        - proof 须为 ES256、64 字节裸 R||S 无填充 base64url，覆盖除
          proof 外八字段并加入 tenant_id=source_tenant_id 的递归键
          升序紧凑 UTF-8 JSON；格式错、验签错、到期依次返回
          “签名格式错误”/“签名校验失败”/“证明已过期”，较早错误优先。

        校验顺序：证明 -> 挑战 -> 锚点 -> 签名格式 -> 验签 -> 期限。
        ...
        # 证明字段（沿用 /v1/trust/proofs/verify 的九字段分类原因）。
        required_fields = {
            "proof_id",
            "credential_id",
            "issuer_did",
        ...
        # 同步锚点：(issuer_did, 版本) 最后事件，须 active 且含 proof 用途。
        ...
        # 签名格式（严格：64 字节裸 R||S 无填充 base64url）与密码学
        # 验签：覆盖除 proof 外八字段，并加入
        # tenant_id=source_tenant_id 后的规范化 JSON。
