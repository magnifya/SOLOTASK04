...
class VCStore:
    ...
    def verify_trust_presentation_with_status(
        ...
        先按 :meth:`verify_trust_presentation` 的全部规则完成外部演示
        验真（请求结构 → 演示字段与挑战 → 签发者锚点与签名 → 持有者
        锚点与签名（仅绑定形态）→ 期限），失败原样返回
        ``(False, 原因)``，保持既有优先级与中文原因分类；未绑定与
        持有者绑定两种形态（含 source_tenant_id、双锚点双签名）均沿用
        单项接口。
        ...
        纯只读：不消费、不登记 DID/凭证/演示，不写状态、历史或审计。
        """
        valid, reason = self.verify_trust_presentation(tenant_id, data)
        if not valid:
            return False, reason
    ...
    def verify_trust_presentations_batch_with_status(
        ...
        请求级合法时逐项复用
        :meth:`verify_trust_presentation_with_status`（与单项验真一致的
        未绑定与持有者绑定形态、字段、挑战、双锚点双签名与期限规则，
