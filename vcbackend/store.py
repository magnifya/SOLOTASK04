...
class VCStore:
    ...
    def verify_trust_presentation(
        ...
        （绑定时）持有者锚点与签名 -> 期限。
        - 未绑定请求体须恰含 presentation（对象）与 challenge（非空字符
          串）；持有者绑定请求体恰含这两项及非空字符串 source_tenant_id；
        - 未绑定演示对象须恰为九字段（presentation_id、credential_id、
          issuer_did、issuer_key_version、disclose、claims、challenge、
          expires_at、proof），出现任何 holder_* 字段即失败；绑定演示
          对象须恰为九字段外加 holder_did（非空字符串）、
          holder_key_version（正整数）、holder_proof（非空字符串）；
          请求 challenge 须等于演示 challenge；
        - 锚点按本租户 (did, key_version) 查找，仅 active 的 P-256 公钥
          可用，缺失或 revoked 失败；签发者与持有者分别按各自
          DID/版本匹配本租户锚点；
        - proof 为 ES256/SHA-256，64 字节裸 R||S 的无填充 base64url，
          覆盖对象中除 proof 与 holder_* 外全部字段的递归排序紧凑
          JSON（覆盖范围不随持有者绑定改变）；holder_proof 同为 ES256，
          覆盖去掉 proof、holder_proof 的绑定对象并加入
          tenant_id=source_tenant_id 后规范化；
        - expires_at 须为 UTC 秒精度 Z 格式，当前时间达到它即过期。
        只读：不登记 DID/凭证/演示，不写状态、历史或审计，绝不向上抛异常。
        """
        # 1. 请求结构：未绑定恰含 presentation/challenge；绑定另含非空
        # 字符串 source_tenant_id。
        if not isinstance(data, dict):
            return False, "请求不合法: 请求体必须为 JSON 对象"
        ...
        if is_holder_bound and (
            not isinstance(source_tenant_id, str) or not source_tenant_id
        ):
            return False, "请求不合法: 字段 source_tenant_id 必须为非空字符串"
        if not isinstance(presentation, dict):
            return False, "请求不合法: 字段 presentation 必须为 JSON 对象"
        ...
        if is_holder_bound:
            ...
            if (
                not isinstance(presentation["holder_proof"], str)
                or not presentation["holder_proof"]
            ):
        ...
        # 4/5. 签名格式与密码学验签：issuer proof 覆盖对象中除 proof 与
        # holder持有者绑定字段外的全部字段（覆盖范围不随绑定改变）。
        ...
        # 持有者绑定：持有者锚点（本租户 active）与 holder_proof 验签，
        # 在签发者验签之后、期限检查之前。
        if is_holder_bound:
            holder_did = presentation["holder_did"]
            holder_version = presentation["holder_key_version"]
            with self._lock:
                bucket = self._bucket_locked(tenant_id)
                holder_anchors = (
                    bucket["trust_anchors"].get(holder_did)
                    if bucket is not None else None
                )
                holder_row = (
                    holder_anchors.get(str(holder_version))
            ...
            if holder_row is None:
                return False, (
                    f"持有者锚点不存在: {holder_did}#{holder_version}"
                )
            if holder_status == "revoked":
                return False, (
                    f"持有者锚点已吊销: {holder_did}#{holder_version}"
                )
            try:
                crypto.validate_public_key_pem(holder_public_pem)
            except (ValueError, TypeError):
                return False, (
                    f"持有者锚点公钥不可用: {holder_did}#{holder_version} "
                    "不是合法 P-256 公钥"
                )
            # holder_proof 覆盖去掉 proof、holder_proof 的绑定对象，并
            # 加入 tenant_id=source_tenant_id 后规范化。
            holder_message = {
                k: v
                for k, v in presentation.items()
