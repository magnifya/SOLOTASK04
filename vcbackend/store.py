...
class VCStore:
    ...
    def verify_credential(
        ...
        缺版本按 1 处理）。任何失败都返回带类别前缀的非空中文原因
        （请求/资源/锚定/密钥/签名），绝不抛异常、绝不泄露私钥。
        """
        if not isinstance(body, dict):
            return False, "请求错误: 字段 body 必须为 JSON 对象"
        if not isinstance(signature, str) or not signature:
            return False, "请求错误: 字段 signature 必须为非空字符串"

        with self._lock:
            rec = self._credentials.get(credential_id)
            if rec is None:
                return False, f"资源错误: 凭证不存在: {credential_id}"
            stored_body = rec["body"]

            if body.get("credential_id") != stored_body.get("credential_id"):
                return False, "锚定错误: 正文 credential_id 与存储记录不一致"
            if body.get("issuer_did") != stored_body.get("issuer_did"):
                return False, "锚定错误: 正文 issuer_did 与存储记录不一致"
            stored_version = stored_body.get("issuer_key_version", 1)
            if body.get("issuer_key_version", 1) != stored_version:
                return False, (
                    "锚定错误: 正文 issuer_key_version 与存储记录不一致"
                )
            ...
            if not public_pem:
                return False, (
                    f"密钥错误: 签发者 {issuer_did} 密钥版本 "
                    f"{stored_version} 的历史公钥不可用"
                )

        try:
            crypto.verify(body, signature, public_pem)
        except crypto.SignatureFormatError as exc:
            return False, f"签名错误: 签名格式非法（{exc}）"
        except crypto.InvalidSignature:
            return False, "签名错误: 签名校验失败，正文或签名可能被改动"
        except (ValueError, TypeError):
            return False, "密钥错误: 验签公钥不可用或无法解析"
        except Exception:  # noqa: BLE001 验签绝不向上抛错
            return False, "签名错误: 验签过程发生内部错误"
        return True, ""
