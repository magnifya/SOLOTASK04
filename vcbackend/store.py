...
class VCStore:
    ...
    def _synced_anchor_events_latest(
        ...
        """读取该来源 cursor<=at 的已同步锚点事件末项视图（只读）。

        该签名方在本租户无同步检查点（含跨租户）抛 NotFoundError；at
        超过检查点抛 ConflictError。返回 (did, key_version) -> 最后
    ...
    def _verify_synced_credential_against(
        ...
        """在已解析的同步锚点快照上验真一条外部凭证（只读）。

        假定 body 为 JSON 对象、signature 为非空字符串；凭证字段、
        锚点、签名格式、验签与有效期规则及原因分类与
    ...
    def verify_trust_credential_synced(
        ...
        return self._verify_synced_credential_against(latest, body, signature)
    ...
    def _verify_synced_presentation_against(
        ...
        假定 presentation 为 JSON 对象、challenge 为非空字符串。未绑定
        形态（source_tenant_id 为 None）的演示字段、挑战、锚点、签名
        格式、验签与期限规则及原因分类与
        :meth:`verify_trust_presentation_synced` 完全一致，供单条与
        批量同步锚点验真共用同一快照。持有者绑定形态
        （source_tenant_id 为非空字符串）演示恰为绑定十二字段，签发者
        与持有者锚点均取同一快照的 (DID, 版本) 末事件，须 active 且
        uses 含 vp；持有者锚点不可用为“同步持有者锚点不可用”，
        holder_proof 覆盖去掉 proof、holder_proof 的对象并加入
        tenant_id=source_tenant_id，格式、验签失败依次为
        “持有者签名格式错误”“持有者签名校验失败”。校验顺序：演示
        字段 -> 挑战 -> 签发锚点/格式/验签 -> 持有者锚点/格式/验签
        （仅绑定）-> 期限。
        """
        # 演示字段（沿用 /v1/trust/presentations/verify 的分类原因）：
        # 未绑定恰为九字段，出现 holder_* 即失败；绑定恰为九字段外加
        ...
        # 持有者绑定：持有者锚点取同一同步快照的 (holder_did, 版本)
        # 末事件，须 active 且含 vp 用途；holder_proof 覆盖去掉
        # proof、holder_proof 的绑定对象并加入
        # tenant_id=source_tenant_id 后规范化验签。
        if is_holder_bound:
            holder_anchor = latest.get((holder_did_value, holder_version_value))
            if (
    ...
    def verify_trust_presentations_synced_batch_with_status(
        self,
        tenant_id: str,
        signer_did: str,
        at: int,
        ...
        """批量以同步锚点快照验真未绑定外部演示并合并批初状态快照。

        请求级结构（恰含 signer_did/at/presentations、类型与 1–100 项
        上限）由服务层校验（400）。本方法假定三者类型已合法：
        - 检查点语义与 :meth:`verify_trust_presentations_synced_batch`
          一致：来源未同步（含跨租户）抛 NotFoundError，at 超检查点
          抛 ConflictError，均先于逐项校验；合法时按 cursor<=at 解析
          一次锚点快照，并在批初同一次原子读取本租户
          ``credential_status_sync`` 状态快照，整批共用；
        - 逐项不短路、等长同序：项须恰含 presentation（JSON 对象）与
          challenge（非空字符串），否则该项
          ``{"valid": false, "reason": "请求项非法"}``；合法项复用
          单条 verify-synced 的未绑定九字段、holder_* 禁令、挑战、
          cursor<=at 末锚点、签名覆盖、格式、验签与期限规则及原因分类；
        ...
        for item in items:  # 逐项处理，失败不短路
            ...
            valid, reason = self._verify_synced_presentation_against(
                latest, presentation, item["challenge"]
            )
            if valid:
                row = status_rows.get(
    ...
    def verify_trust_presentation_synced_with_status(
        ...
        """以同步锚点验真外部演示并合并请求初始状态快照（只读）。

        请求结构（恰含 signer_did/at/presentation/challenge，持有者绑
        定另恰含非空字符串 source_tenant_id，及类型）由服务层校验
        （400）。检查点语义、演示字段、挑战、锚点、签名覆盖、格式、
        验签、期限规则、校验顺序及原因分类与
        :meth:`verify_trust_presentation_synced` 完全一致；绑定形态
        演示恰为十二字段，签发者与持有者锚点均取同一 cursor<=at 同步
        快照的 (DID, 版本) 末事件（须 active 且 uses 含 vp），持有者
        锚点不可用为“同步持有者锚点不可用”，holder_proof 覆盖去掉
        proof、holder_proof 的对象并加入 tenant_id=source_tenant_id，
        格式、验签失败依次为“持有者签名格式错误”“持有者签名校验
        失败”，顺序为演示、挑战、签发锚点/格式/验签、持有者锚点/
        格式/验签、期限。验真失败原样返回，不查状态。验真通过后按
        演示的 ``(issuer_did, credential_id)`` 查请求初始一次原子读
        取的本租户 ``credential_status_sync`` 状态快照：未同步（含属
        他租户）为“外部凭证状态未同步”，revoked 为“外部凭证已吊销：
    ...
    def verify_trust_proof_synced(
        ...
        """以同步锚点验真外部谓词证明，返回 (是否有效, 失败原因)。

        请求结构（恰含 signer_did/at/proof/challenge/source_tenant_id
        及类型）由服务层校验（400）。本方法假定五键类型已合法：
        - 该签名方在本租户无同步检查点（含跨租户）抛 NotFoundError；
          at 超过检查点抛 ConflictError；
        - 证明九字段、predicates、results、challenge 与 expires_at
          规则沿用 :meth:`verify_trust_proof`；
        - 锚点取该来源 cursor<=at 的已同步事件，以 (issuer_did, 版本)
          最后事件为准；缺失、非 active 或 uses 无 proof 均返回
          (False, "同步锚点不可用")；
        - proof 须为 ES256、64 字节裸 R||S 无填充 base64url，覆盖除
          proof 外八字段并加入 tenant_id=source_tenant_id 的规范化
          JSON；格式错、验签错、到期依次返回 “签名格式错误”/
          “签名校验失败”/“证明已过期”，较早错误优先。

        校验顺序：请求 -> 证明字段 -> 挑战 -> 锚点 -> 签名格式 ->
    ...
    def _verify_synced_proof_against(
        ...
        # 挑战：请求 challenge 须等于证明 challenge。
        if challenge != proof["challenge"]:
            return False, "挑战不匹配: 请求 challenge 与证明 challenge 不一致"

        # 同步锚点：(issuer_did, 版本) 最后事件，须 active 且含 proof
        # 用途。
        issuer_did = proof["issuer_did"]
        anchor = latest.get((issuer_did, key_version))
        if (
        ...
        # 签名格式（严格：64 字节裸 R||S 无填充 base64url）与密码学
        # 验签：覆盖范围沿用 /v1/trust/proofs/verify——去掉 proof 后的
        # 八个字段并加入 tenant_id=source_tenant_id。
        signature = proof["proof"]
        message = {k: v for k, v in proof.items() if k != "proof"}
        message["tenant_id"] = source_tenant_id
        try:
    ...
    def verify_trust_proofs_synced_batch_with_status(
        self,
        tenant_id: str,
        signer_did: str,
        at: int,
        ...
        """批量以同步锚点快照验真外部谓词证明并合并批初状态快照。

        请求级结构（恰含 signer_did/at/proofs、类型与 1–100 项上限）
        由服务层校验（400）。本方法假定三者类型已合法：
        - 检查点语义与 :meth:`verify_trust_proofs_synced_batch` 一致：
          来源未同步（含跨租户）抛 NotFoundError，at 超检查点抛
          ConflictError，均先于逐项校验；合法时按 cursor<=at 解析一次
          锚点快照，并在批初同一次原子读取本租户
          ``credential_status_sync`` 状态快照，整批共用；
        - 逐项不短路、等长同序：项须恰含 proof（JSON 对象）、
          challenge（非空字符串）、source_tenant_id（非空字符串），
          否则该项 ``{"valid": false, "reason": "请求项非法"}``；合法
          项复用单条 verify-synced 的证明九字段、谓词/results、
          RFC6901 路径、挑战、cursor<=at 末锚点、签名覆盖、格式、验签
          与期限规则及原因分类；
    ...
    def verify_trust_did_document(
        ...
        # 2. 文档结构：四字段、验证方法、P-256 PEM、禁私钥、升序无重、
        #    当前版本即最高版本。
        did, current_version, highest_public_pem, doc_reason = (
            self._check_did_document_fields(document)
        )
        if doc_reason:
            return False, doc_reason

        # 3. 锚点：当前租户同 DID、同版本、active 且公钥原文完全匹配
        with self._lock:
            bucket = self._bucket_locked(tenant_id)
            anchors = (
                bucket["trust_anchors"].get(did)
        ...
        # 4. 签名格式与密码学验签：证明覆盖除 document_proof 外的整个文档
        ...
        # 5. 原验真成功后查本租户外部 DID 停用通告：命中同 did 返回
    ...
    @staticmethod
    def _check_did_document_fields(
        document: Any,
    ) -> Tuple[str, int, str, str]:
        """校验 DID 文档结构（只读），供本地锚点与同步锚点验真共用。

        规则与 :meth:`verify_trust_did_document` 的文档结构部分完全一致：
        document 须恰含 did（非空字符串）、current_key_version（非布尔正
        整数）、verification_methods（非空数组）、document_proof（非空
        字符串）；方法按 key_version 严格升序且无重复，每项恰含
        key_version（非布尔正整数）、key_handle（非空字符串）、
        public_key（可解析 P-256 公钥 PEM），文档任何位置不得出现私钥；
        current_key_version 必须等于最高方法版本。成功返回
        ``(did, current_version, 最高版本 public_key, "")``，失败返回
        ``("", 0, "", 非空中文原因)``。
        """
        required = (
            "did",
        ...
        if missing:
            return "", 0, "", f"DID文档缺少字段: {', '.join(missing)}"
        extra = sorted(set(document) - set(required))
        if extra:
            return "", 0, "", f"DID文档含多余字段: {', '.join(extra)}"

        did = document["did"]
        if not isinstance(did, str) or not did:
            return "", 0, "", "DID文档字段 did 必须为非空字符串"
        ...
        if (
            ...
            return "", 0, "", (
                "DID文档字段 current_key_version 必须为非布尔正整数"
            )

        proof = document["document_proof"]
        if not isinstance(proof, str) or not proof:
            return "", 0, "", (
                "DID文档字段 document_proof 必须为非空字符串"
            )

        methods = document["verification_methods"]
        if not isinstance(methods, list):
            return "", 0, "", "DID文档字段 verification_methods 必须为数组"
        if not methods:
            return "", 0, "", "DID文档 verification_methods 必须为非空数组"
        ...
        for index, method in enumerate(methods):
            ...
            if not isinstance(method, dict):
                return "", 0, "", f"DID文档{label}必须为 JSON 对象"
        ...
        try:
            ...
            return "", 0, "", "DID文档含无法序列化的内容"
        if "PRIVATE KEY-----" in serialized:
            return "", 0, "", "DID文档不得包含私钥"
        ...
        return did, current_version, methods[-1]["public_key"], ""
    ...
    def verify_trust_did_document_synced(
        ...
        """以同步锚点快照验真外部 DID 文档，返回 (是否有效, 失败原因)。

        请求结构（恰含 signer_did/at/document 及类型）由服务层校验
        （400）。本方法假定 document 为 JSON 对象：
        - 该签名方在本租户无同步检查点（含跨租户）抛 NotFoundError；
          at 超过检查点抛 ConflictError，均先于文档结构校验；
        - document 四字段、验证方法升序无重、P-256 PEM、禁私钥及当前
          版本即最高版本规则与 :meth:`verify_trust_did_document` 完全
          一致，失败返回“DID文档非法”；
        - 锚点取该来源 cursor<=at 的已同步事件，以 (did, 当前版本)
          最后事件为准；缺失、非 active、uses 无 did 或事件公钥与文档
          当前版本 public_key 非逐字相同均返回
          (False, "同步锚点不可用")；
        - document_proof 沿用现有 DID 文档验真协议：ES256 裸 R||S 无
          填充 base64url，覆盖除 document_proof 外整个文档的规范化
          JSON；格式错返回“签名格式错误”，验签失败返回“签名校验失败”；
        - 验签成功后查本租户同 did 外部 DID 停用通告，命中返回
          “外部DID已停用：<reason>”，否则成功。

        纯只读：不改同步页、检查点、锚点、状态或审计，结论随状态文件
        跨重启稳定。
        """
        latest = self._synced_anchor_events_latest(tenant_id, signer_did, at)
        ...
        if (
            ...
            or "did" not in (anchor.get("uses") or [])
        ):
            return False, SYNCED_ANCHOR_UNAVAILABLE_REASON
        anchor_public_pem = anchor.get("public_key", "")
        if anchor_public_pem != highest_public_pem:
            return False, SYNCED_ANCHOR_UNAVAILABLE_REASON
        try:
            crypto.validate_public_key_pem(anchor_public_pem)
        except (ValueError, TypeError):
            return False, SYNCED_ANCHOR_UNAVAILABLE_REASON
