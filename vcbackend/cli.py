...
子命令：
  ...
  verify CREDENTIAL_ID                   调服务端验签接口，输出 true/false
...
def _cmd_verify(args: argparse.Namespace) -> int:
    # 现取凭证签名，再交由服务端以存储的锚点（credential_id、
    # issuer_did、issuer_key_version 及历史公钥）完成验签。
    vc = _request(
    ...
    if result.get("valid") is True:
        print("true")
        return 0
    reason = result.get("reason") or "签名校验失败"
    print("false")
