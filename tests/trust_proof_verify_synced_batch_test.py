#!/usr/bin/env python3
"""POST /v1/trust/proofs/verify-synced-batch 批量同步锚点验真谓词证明测试。
...
- 404/409 先于项校验：来源未同步或跨租户 404 仅 {"error":"同步来源
  不存在"}，at 超过检查点 409 仅 {"error":"同步游标冲突"}；
- 200 逐项：等长同序、不短路；项须恰含 proof/challenge/
  source_tenant_id，否则 {"valid":false,"reason":"请求项非法"}；合法
  项沿用单条 verify-synced 的证明九字段、谓词、results、挑战、锚点、
  签名覆盖、格式、验签与期限规则及原因分类，较早错误优先；成功项仅
...
def make_item(proof, priv, challenge=CHALLENGE, source=SOURCE_TENANT):
    return {
        "proof": sign_proof(proof, priv, source),
        "challenge": challenge,
...
def test_http():
    ...
    def expect_error(name, st, raw, status, error):
        try:
    ...
    def good_payload(items, at=1, signer=signer_did):
    ...
    try:
        ...
        good_item = make_item(make_proof(issuer_did), issuer_priv)
        ...
        expect_400("proofs 非数组 400",
                   payload={"signer_did": signer_did, "at": 1,
                            "proofs": {}})
        expect_400("proofs 空数组 400",
                   payload={"signer_did": signer_did, "at": 1,
                            "proofs": []})
        expect_400("proofs 超上限 400",
                   payload={"signer_did": signer_did, "at": 1,
                            "proofs": [good_item] * 101})
        st, raw = batch(payload=good_payload([good_item]),
                        headers={"X-Tenant-ID": ""})
        check("显式空租户头 400",
              st == 400
        ...
        st, raw = batch(payload=good_payload([good_item]), headers=TC)
        expect_error("跨租户未同步 404", st, raw, 404, "同步来源不存在")
        st, raw = batch(payload=good_payload([good_item], at=5))
        expect_error("at 超过检查点 409", st, raw, 409, "同步游标冲突")
        st, raw = batch(payload=good_payload([good_item], at=3), headers=TB)
        expect_error("tenant-b at 超过其检查点 409", st, raw, 409,
                     "同步游标冲突")
        # 409 优先于项校验：项本身非法也仍 409
        st, raw = batch(payload=good_payload([{"bogus": 1}], at=5))
        expect_error("at 超检查点优先于项校验 409", st, raw, 409,
                     "同步游标冲突")
        st, raw = batch(payload=good_payload([{"bogus": 1}],
                                             signer="did:web:nobody"))
        expect_error("未同步优先于项校验 404", st, raw, 404, "同步来源不存在")

        # ---------------------------------------------------------- #
        # 3. 200：项结构非法 -> 请求项非法（不短路、等长同序）
        # ---------------------------------------------------------- #
        st, raw = batch(payload=good_payload([
            good_item,
            {"proof": {}, "challenge": CHALLENGE},  # 缺 source_tenant_id
            {"proof": {}, "challenge": CHALLENGE,
             "source_tenant_id": SOURCE_TENANT, "x": 1},  # 多余键
            {"proof": [], "challenge": CHALLENGE,
             "source_tenant_id": SOURCE_TENANT},  # proof 非对象
            {"proof": {}, "challenge": "",
             "source_tenant_id": SOURCE_TENANT},  # challenge 空串
            {"proof": {}, "challenge": CHALLENGE,
             "source_tenant_id": ""},  # source_tenant_id 空串
            "not-an-object",
        ]))
        body = json.loads(raw)
        check("混合项 200 仅 results 键", st == 200
              and list(body.keys()) == ["results"])
        results = body["results"]
        check("等长同序", len(results) == 7)
        check("首项成功仅 valid:true", results[0] == {"valid": True})
        check("其余各项请求项非法",
              all(item == {"valid": False, "reason": "请求项非法"}
                  for item in results[1:]))

        # ---------------------------------------------------------- #
        # 4. 200：合法项沿用单条 verify-synced 规则与原因分类
        # ---------------------------------------------------------- #
        no_pid = make_proof(issuer_did)
        del no_pid["proof_id"]
        bad_sig_item = make_item(make_proof(issuer_did), issuer_priv)
        bad_sig_item["proof"]["proof"] = "not-a-signature"
        tampered = sign_proof(make_proof(issuer_did), issuer_priv)
        tampered["results"] = [False]
        st, raw = batch(payload=good_payload([
            make_item(no_pid, issuer_priv),  # 证明字段错误
            make_item(make_proof(issuer_did), issuer_priv,
                      challenge="other"),  # 挑战不匹配
            make_item(make_proof("did:web:unknown"), issuer_priv),  # 无锚点
            make_item(make_proof("did:web:noproof"), issuer_priv),  # 无proof用途
            bad_sig_item,  # 签名格式错误
            {"proof": tampered, "challenge": CHALLENGE,
             "source_tenant_id": SOURCE_TENANT},  # 签名校验失败
            make_item(make_proof(issuer_did,
                                 expires_at="2020-01-01T00:00:00Z"),
                      issuer_priv),  # 证明已过期
            make_item(make_proof(issuer_did, issuer_key_version=2),
                      issuer_priv2),  # v2 在 at=1 尚未注册
        ]))
        ...
        check("逐项原因分类与顺序",
              [item.get("reason") for item in results] == expected
              and all(list(item.keys()) == ["valid", "reason"]
                      and item["valid"] is False for item in results))

        # 较早错误优先：锚点不可用优先于签名格式错误
        bad_anchor = sign_proof(make_proof("did:web:unknown"), issuer_priv)
        bad_anchor["proof"] = "not-a-signature"
        st, raw = batch(payload=good_payload([
            {"proof": bad_anchor, "challenge": CHALLENGE,
             "source_tenant_id": SOURCE_TENANT},
        ]))
        check("锚点不可用优先于签名格式错误",
              json.loads(raw)["results"] == [
                  {"valid": False, "reason": "同步锚点不可用"}])

        # ---------------------------------------------------------- #
        # 5. 成功：at 快照语义（at=4 用 v2，at=3 issuer#1 已吊销）
        # ---------------------------------------------------------- #
        st, raw = batch(payload=good_payload([
            make_item(make_proof(issuer_did, issuer_key_version=2),
                      issuer_priv2),
        ...
        batch(payload=good_payload([good_item, good_item]))
        batch(payload=good_payload([good_item], at=5))
        batch(payload=good_payload([{"bogus": 1}]))
        ...
        # 7. 重启一致
        # ---------------------------------------------------------- #
        st, ok_before = batch(payload=good_payload([good_item]))
        st, na_before = batch(payload=good_payload([good_item], at=3))
        proc.terminate()
        proc.wait(timeout=10)
        proc = start()
        st, raw = batch(payload=good_payload([good_item]))
        check("重启后成功响应逐字节一致", st == 200 and raw == ok_before)
        st, raw = batch(payload=good_payload([good_item], at=3))
