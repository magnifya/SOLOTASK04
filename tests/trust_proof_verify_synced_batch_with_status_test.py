...
- 200 仅 {"results":[...]}，逐项不短路、等长同序：项结构非法为
  ...
  （空或缺失 reason 用“未知原因”）“外部凭证状态未知”，active 成功；
  成功项仅 {"valid":true}，失败项键序 valid、reason；
- 纯只读：不推进检查点、不改同步页/锚点/证明/状态/历史、不记审计；
...
def make_proof(issuer_did, credential_id="vc-ext-1", **overrides):
    proof = {
        "proof_id": "prf-ext-1",
...
def test_http():
    ...
    def item(cred_id="vc_active", issuer=issuer_did, priv=issuer_priv,
             challenge=CHALLENGE, source=SOURCE_TENANT, **proof_overrides):
        proof = make_proof(issuer, credential_id=cred_id, **proof_overrides)
    ...
    def history_events(cred_id):
        st, raw = _http(
            "GET",
            f"{base}/v1/trust/credential-status/{cred_id}/history"
    ...
    try:
        ...
        good_proof = make_proof(issuer_did, credential_id="vc_active")
        good_item = item()
        ...
        # ---------------------------------------------------------- #
        no_pid = make_proof(issuer_did, credential_id="vc_active")
        del no_pid["proof_id"]
        tampered_item = item()
        ...
        items = [
            ...
            "not-an-object",                                  # 非对象项
            {**item(), "proof": sign_proof(no_pid, issuer_priv)},  # 证明缺字段
        ...
        check("结果等长", len(body["results"]) == len(items))
        ...
        # 吊销原因空串同样归一为“未知原因”：同步入口拒绝空 reason
        # （400），但持久化状态允许存在空串历史行，故停服直接改写
        # 状态文件再重启，验真须归一处理。
        proc.terminate()
        proc.wait(timeout=10)
        with open(store_path, "r", encoding="utf-8") as fh:
            state = json.load(fh)
        row = dict(state["tenants"]["tenant-a"]["credential_status_sync"][
            issuer_did]["vc_rev_nr"])
        row["reason"] = ""
        state["tenants"]["tenant-a"]["credential_status_sync"][
            issuer_did]["vc_rev_empty"] = row
        with open(store_path, "w", encoding="utf-8") as fh:
            json.dump(state, fh, ensure_ascii=False)
        proc = start()
        st, raw = verify(payload=batch([item("vc_rev_empty")], at=1))
        check("空串吊销原因归一为未知原因",
              st == 200 and json.loads(raw) == {"results": [
                  {"valid": False,
                   "reason": "外部凭证已吊销：未知原因"}]})
        ...
        st, state_before = _http(
            ...
            headers=TA)
        assert st == 200
        history_before = history_events("vc_active")
        ...
        check("批量验真不写状态历史",
              history_events("vc_active") == history_before)
